...
class CrimeAnalysis:
    ...
    def _get_county_crime_data(self, address: str, lat: float, lon: float,
        ...
        # Branch: file-based vs API-based data source
        try:
            if self.config.crime_data_source == "file":
                incidents = self._query_crime_file(lat, lon, radius_miles,
                                                   start_date, end_date)
            else:
            ...
            if incidents is not None:
                ...
                result.data_source = (
                    'FOIA Crime Data File' if self.config.crime_data_source == "file"
                    else 'County Sheriff Crime Dashboard'
                )
                ...
                result.notes = "Check data source and date range"

        except Exception as e:
            result.error_message = f"Error querying crime data: {str(e)}"
            result.notes = "Check data source and connectivity"
    ...
    def _query_crime_file(self, lat: float, lon: float,
                          radius_miles: float, start_date: datetime,
                          end_date: datetime) -> Optional[List[CrimeIncident]]:
        """
        Query crime data from a local parquet file.

        Loads the processed incidents parquet, filters by date range,
        then filters by haversine distance from the query point.

        Returns:
            List of CrimeIncident objects within the radius and date range,
            or None if the file cannot be loaded.
        """
        file_path = self.config.crime_data_file_path
        if not file_path:
            return None

        # Resolve relative to the multi-county-real-estate-research directory
        data_path = Path(__file__).parent.parent / file_path
        ...
        # Filter by radius (only for geocoded records)
        if "latitude" in df.columns and "longitude" in df.columns:
            geocoded = df.dropna(subset=["latitude", "longitude"])
            if len(geocoded) > 0:
                geocoded = geocoded.copy()
                geocoded["_dist"] = geocoded.apply(
                    lambda r: self._haversine_distance(
                        lat, lon, r["latitude"], r["longitude"]
                    ), axis=1
                )
                df = geocoded[geocoded["_dist"] <= radius_miles]

        # Convert to CrimeIncident objects
        incidents = []
        for _, row in df.iterrows():
            incidents.append(CrimeIncident(
                incident_id=str(row.get("case_number", "")),
                incident_type=str(row.get("offense_category", row.get("category", ""))),
                date=row["_date"].to_pydatetime() if pd.notna(row.get("_date")) else datetime.now(),
                address=str(row.get("block_address", row.get("address", ""))),
                lat=row.get("latitude") if pd.notna(row.get("latitude")) else None,
                lon=row.get("longitude") if pd.notna(row.get("longitude")) else None,
                jurisdiction=str(row.get("station", "LCSO")),
            ))
